/*
 * Module to calculate the Sunrise, Sunset and Solar Noon
 *
 * Example use:
...
// flag=2 for DD MM, 3 for DD MM YYYY, 4 for DDMMYYYY next/prev
function dayString(jd, next, flag) {
  ...
  var output = date.day + '/' + (day.month - 1) + '/' + date.year;
  if (flag == 2)
    output = zeroPad(date.day, 2) + " " + zeroPad(date.month - 1, 2);
  if (flag == 3)
    output = zeroPad(date.day, 2) + zeroPad(date.month - 1, 2).abbr + date.year.toString();
  if (flag == 4)
    output = zeroPad(date.day, 2) + zeroPad(date.month - 1, 2).abbr + date.year.toString() + ((next) ? " next" : " prev");
...
// flag=2 for HH:MM, 3 for HH:MM:SS
exports.solarNoonAsString = function(date, lon, flag) {
  if (! flag) flag = 3;
  var jday = getJD(date.getFullYear(), date.getMonth() + 1, date.getDate());
  var tz = date.getTimezoneOffset() / 60 * -1;
  var solnoon = calcSolNoon(jday, lon, tz);
  return timeString(solnoon, flag);
};


function sunRiseOrSetToString(rise, date, lat, lon) {
  var jday = getJD(date.getFullYear(), date.getMonth() + 1, date.getDate());
  var tz = date.getTimezoneOffset() / 60 * -1;
  var solData = calcSunriseSet(rise, jday, lat, lon, tz);
  var output = '';
  if (solData.jday == jday) {
    output = timeString(solData.timelocal, 2);
  } else {
    if (solData.azimuth >= 0.0) {
      output = timeDateString(solData.jday, solData.timelocal);
    } else {
      output = dayString(solData.jday, 0, 3);
    }
  }
  return output;
}

exports.sunriseAsString = function(date, lat, lon) {
  return sunRiseOrSetToString(true, date, lat, lon);
};

exports.sunsetAsString = function(date, lat, lon) {
  return sunRiseOrSetToString(false, date, lat, lon);
};
